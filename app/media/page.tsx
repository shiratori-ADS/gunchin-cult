...
export default function MediaPage() {
  ...
  return (
    <PageShell>
      <SectionTitle eyebrow={siteContent.media.eyebrow} title={siteContent.media.title} />

      <div className="mx-auto grid max-w-6xl gap-5 md:grid-cols-2">
        {siteContent.media.items.map((item, index) => {
          const thumbnail = item.image.trim();
          const detailSrc = item.detailImage.trim() || thumbnail;
          const detailAlt = item.detailImageAlt.trim() || item.imageAlt;
            ...
            >
              {thumbnail ? (
                <button
                  ...
                  onClick={() => setExpandedImage({ src: detailSrc, alt: detailAlt })}
                  aria-label={`${detailAlt}を拡大表示`}
                >
                  <ContentImage
                    image={thumbnail}
              ...
              <h2 className="text-2xl font-black leading-9 text-white">{renderMultiline(item.title)}</h2>
